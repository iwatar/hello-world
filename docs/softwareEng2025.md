...
# 第8回
バージョン管理：<br>
　複数人で共同開発を行う際にとても重要。集中管理・分散管理型がある。<br>
　集中管理型：コンフリクトが発生しやすく同期に時間がかかる。<br>
　分散管理型：障害に強い。<br>
<br>
git：<br>
　分散管理型のバージョン管理システム。変更履歴が残る・変更した箇所に戻れる・共同編集が出来る。<br>
　コミット：ファイル操作の記録。「どのような」のメッセージを残す。<br>
　レポジトリ：gitが管理するプロジェクトのフォルダ。ローカルは個々の、リモートは共有の場所。<br>
　ブランチ：作業の枝分かれ。共同作業・並行作業が可能。<br>

# 第10回
Learning git branchingをした。

# 第11回
github：<br>
　リモートリポジトリのホスティングサービス、オープンソースソフトウェアの主要なポータルサイト。<br>

# 第12回
CI/CD：<br>
　Continuous Integration(CI):<br>
　　コード変更を頻繁に統合するプロセス。自動テスト・ビルドでバグの早期発見と修正<br>
　Continuous Delivery(CD):<br>
　　コード変更を自動的にデプロイするプロセス、ユーザフィードバックを迅速に反映可能<br>
　コード変更をトリガーにワークフローを起動->ビルド->テスト->デプロイ->検証->本番環境での継続的な監視<br>
　github Actionsを使用するとできる。<br>
